...
from .backbones.internvideo2 import InternVideo2_roi_branch, TextTransformer, ClipTokenizer
...
class InternVideo2_CLIP_branch(nn.Module):
    ...
    def encode_vision(self, image, roi_box_list, test=False):
        ...
        roi_embeds = self.roi_align(roi_embeds)
    ...
    def build_vision_encoder(self):
        ...
        """
        vision_encoder = InternVideo2_roi_branch(
    ...
    def load_checkpoint(self, vision_ckpt_path=None, text_ckpt_path=None, extra_ckpt_path=None):
        ...
        if self.config.evaluate:
            ...
            for key in finetuned_state["model"].keys():
                vision_ckpt[key] = finetuned_state["model"][key]
            ...
            for k, v in vision_ckpt.items():
                if k.startswith('clip_decoder.') or k.startswith('mae_decoder.') or k.startswith('final_clip_decoder.'):
                    ...
                    if 'vision_encoder' in k:
                        new_ckpt[k] = v
                    elif "roi_align" in k or "vision_align" in k or "action_classifier" in k or "fc_norm" in k or "temp" in k or "head" in k:
                        new_ckpt[k] = v
                    else:
