...
class PatchEmbed(nn.Module):
    ...
    def Roi_Align_M(self, x, roi_box_list):
    ...
    def forward(self, x, roi_box_list):
        ...
        x = self.proj(x)
        B, C, T, H, W = x.shape
        roi_feature = self.Roi_Align_M(x, roi_box_list)
        ...
        global_atnn_input = self.global_attn(x_input, attn_feature, attn_feature)
        
        x = (1 - self.alpha) * x_input + self.alpha * global_atnn_input
        # 请思考这一步，如何将attn_feature合并到后续的步骤中呢？
        # 在经过Patchembed处理过后的特征还会加上位置编码，在给出合并方案的同时请确保特征与各种编码的对齐
        x = x.view(B, T, H * W, C)
        # x = attn_feature.view(B, T, H * W, C)  # B x C x T x HW => B x T x HW x C
        x = self.norm(x)
        return x

class PatchEmbed3D(nn.Module):
    """
    与你原先的 PatchEmbed 类似，但把它做成可配置化的单分支 3D 卷积 patch embed。
    """
...
class MultiScalePatchEmbed(nn.Module):
    def __init__(self,
        ...
        self.patch_embed_low = PatchEmbed3D(
            img_size=img_size,
            patch_size=patch_size_low,
            in_chans=in_chans,
            embed_dim=embed_dim_low,
            num_frames=num_frames,
            tubelet_size=tubelet_size,
            norm_layer=norm_layer
        )
    ...
    def forward(self, x):
        x_high, shape_high = self.patch_embed_high(x)
        x_low, shape_low = self.patch_embed_low(x)
        return x_high, x_low, shape_high, shape_low
...
class CrossScaleFusion(nn.Module):
    ...
    def forward(self, x_high, x_low):
        ...
        x_high_fused = x_high + h2l  # 残差或直接替换，看你需求
...
class InternVideo2_roi(nn.Module):
    def __init__(
        ...
        self.norm_high = nn.LayerNorm(embed_dim_high)
        self.norm_low = nn.LayerNorm(embed_dim)

        self.avg_pool_high = nn.AdaptiveAvgPool1d(1)
        self.avg_pool_low = nn.AdaptiveAvgPool1d(1)
    ...
    def forward(self, x, roi_box_list, use_image=False):
        x_high, x_low, shape_high, shape_low = self.multi_scale_patch_embed(x)
        
        x = self.patch_embed(x.type(self.dtype), roi_box_list)
