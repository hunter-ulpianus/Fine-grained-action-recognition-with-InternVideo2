...
class crossattention_roi(nn.Module):
    def __init__(self, in_channels_roi, in_channels_global, hidden_dim=128, num_heads=8):
        super().__init__()
        self.num_heads = num_heads
        self.hidden_dim = hidden_dim

        self.query_proj = nn.Linear(in_channels_roi, hidden_dim * num_heads, bias=False)
        self.key_proj = nn.Linear(in_channels_global, hidden_dim * num_heads, bias=False)
        self.value_proj = nn.Linear(in_channels_global, hidden_dim * num_heads, bias=False)

        self.output_proj = nn.Linear(hidden_dim * num_heads, in_channels_roi)

    def forward(self, roi_feat, global_feat):
        BT_r, HW_r, C_r = roi_feat.shape
        BT, HW, C= global_feat.shape

        Q = self.query_proj(roi_feat)
        K = self.key_proj(global_feat)
        V = self.value_proj(global_feat)

        Q = Q.view(BT_r, HW_r, self.num_heads, self.hidden_dim).permute(0, 2, 1, 3)
        K = K.view(BT, HW, self.num_heads, self.hidden_dim).permute(0, 2, 1, 3)
        V = V.view(BT, HW, self.num_heads, self.hidden_dim).permute(0, 2, 1, 3)

        K = K.expand(K.size(0), -1, -1, -1)
        V = V.expand(K.size(0), -1, -1, -1)
        ...
        attn_output = attn_output.view(BT_r, HW_r, self.num_heads * self.hidden_dim)

        output = self.output_proj(attn_output)
        output = output.view(BT_r, HW_r, C_r)
        enhanced_roi_feat = output + roi_feat

        return enhanced_roi_feat
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=14, in_chans=3, embed_dim=768, 
        ...
        # self.roi_attn_block = nn.ModuleList()
        
        """
        for i in range(2):
            self.roi_attn_block.append(
                nn.Sequential(
                CrossAttention(dim=embed_dim, num_heads=8, qkv_bias=True),
                 CrossAttention(dim=embed_dim, num_heads=8, qkv_bias=True)))
        ...
        self.roi_attn = crossattention_roi(in_channels_roi=1024, 
                                           in_channels_global=1024,
                                           hidden_dim=128,
                                           num_heads=8)
        self.global_attn = crossattention_roi(in_channels_roi=1024, 
                                           in_channels_global=1024,
                                           hidden_dim=128,
                                           num_heads=8)
        """
    ...
    def forward(self, x, roi_box_list):
        ...
        """
        B_img, C_img, T_img, W_img, H_img = x.shape # (B, 3, T, 224, 224)
        ...
        """
        blk_global_attn_input = x_input
        blk_attn_feature = roi_input

        
        for blk in self.roi_attn_block:
            blk_attn_feature = blk[0](blk_attn_feature, blk_global_attn_input, blk_global_attn_input)
            blk_global_attn_input = blk[1](blk_global_attn_input, blk_attn_feature, blk_attn_feature)
        
        # roi_input = self.norm_roi(roi_input)
        # x_input = self.norm_global(x_input)
        """

        attn_feature = self.roi_attn(roi_input, x_input)
        global_feature = self.global_attn(x_input, attn_feature)
        # global_attn_input = self.global_attn(x_input, attn_feature, attn_feature)

        """""
        for idx, item in enumerate(mask_list):
            if item == 1:
                global_attn_input[idx] = x_input[idx]
        """

        # global_attn_input = blk_global_attn_input
        # global_attn_input = blk_global_attn_input

        # x = (1 - self.alpha) * x_input + self.alpha * global_feature

        """
        x = self.proj(x)  # [B, embed_dim, T', H', W']
        _, C2, T2, H2, W2 = x.shape
        # flatten -> (B, C2, N) -> (B, N, C2)
        x = x.view(B, C2, -1).transpose(1, 2).contiguous()
        """
        x = global_feature.view(B, T, H * W, C)
        x = self.norm(x)
        return x, (T, H, W)
...
class InternVideo2_roi(nn.Module):
    def __init__(
            self,
            in_chans: int = 3,
            patch_size: int = 14,
        ...
        super().__init__()
        """
        self.multi_scale_patch_embed = MultiScalePatchEmbed(
            img_size=img_size,
            in_chans=in_chans,
            embed_dim_high=embed_dim_high,
            embed_dim_low=embed_dim,
            patch_size_high=patch_size_high,
            patch_size_low=patch_size,
            num_frames=num_frames,
            tubelet_size=tubelet_size
        )
        ...
        self.fusion_module = CrossScaleFusion(
            dim_high=embed_dim_high,
            dim_low=embed_dim,
            num_heads_high=num_heads_high,
            num_heads_low=num_heads_low,
            attn_drop=attn_drop,
            proj_drop=proj_drop,
        )
    """
        ...
        self.patch_embed = PatchEmbed(
            ...
            num_frames=num_frames, tubelet_size=tubelet_size,use_flash_attn=use_flash_attn
        )
        """
        ...
        for i in range(depth):
            if i < depth - 10:
                self.blocks.append(Block(embed_dim, num_heads, mlp_ratio, qkv_bias=qkv_bias,
                  norm_layer=norm_layer_for_blocks,
                  drop_path=dpr[i], init_values=init_values, attn_drop=0.,
                  use_flash_attn=use_flash_attn, use_fused_mlp=use_fused_mlp,
                  fused_mlp_heuristic=fused_mlp_heuristic,
                ...
                self.blocks.append(BlockWithAdapter(embed_dim, num_heads, mlp_ratio, qkv_bias=qkv_bias,
                  norm_layer=norm_layer_for_blocks,
                  drop_path=dpr[i], init_values=init_values, attn_drop=0.,
                  use_flash_attn=use_flash_attn, use_fused_mlp=use_fused_mlp,
                  fused_mlp_heuristic=fused_mlp_heuristic,
    ...
    def forward(self, x, roi_box_list, use_image=False):
        # x_high, _= self.multi_scale_patch_embed(x.type(self.dtype), roi_box_list)

       # xB, Th, Size_h, Ch = x_high.shape
        # x_high = x_high.view(B, Th*Size_h, Ch)
        # ori_x = x
        ...
        # x_high, x_low = self.fusion_module(x_high, x)   # [B, T * Hh * Wh, C],[B, T * Hl * Wl, C]

        # x_low_fused = self.fusion_inject_module(x_low, x_high)

        #此时，x_high, x_low的维度不满足接下来的操作，包括加上位置编码输入到后续网络等等，如何融合不同的分辨率的特征，实现细粒度的特征理解？

        # B, T, L, C = x.shape  # T: temporal; L: spatial
        # x = x.view([B, T * L, C])

        #L = x_low_fused.shape[1] // T
        # append cls token
        cls_tokens = self.cls_token.expand(B, -1, -1)
        # x = torch.cat((cls_tokens, x_low_fused), dim=1)
