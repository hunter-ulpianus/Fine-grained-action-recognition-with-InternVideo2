...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=14, in_chans=3, embed_dim=768, 
        ...
        self.roi_attn = CrossAttention(dim=1408, num_heads=8, qkv_bias=True)
        self.global_attn = CrossAttention(dim=1408, num_heads=8, qkv_bias=True)
...
class InternVideo2_1B_branch(nn.Module):
    def __init__(
            self,
            in_chans: int = 3,
            patch_size: int = 14,
            img_size: int = 224,
            qkv_bias: bool = False,
            drop_path_rate: float = 0.3, # may need ablation
