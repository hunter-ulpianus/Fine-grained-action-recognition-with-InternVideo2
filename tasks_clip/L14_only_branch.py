...
def main_with_ensemble(config, test_loader, model_without_ddp, tokenizer, data_type):
    ...
    # with torch.cuda.amp.autocast(enabled=config.use_half_precision, dtype=data_type), torch.no_grad():
        for image, text, ans, ann, roi_box_list, answer in iterator:
            ...
            # encode image
            pooled_image_feat, roi_feat = model.encode_vision(image, roi_box_list, test=True) # [b, c]

            # probs, idxs = predict_label_loss(pooled_image_feat, pooled_text_feat, top=1)
            ...
            for q_idx in range(len(roi_score)):  # bsz
                _pred = dict(
                    ...
                    pred_scores=roi_score[q_idx].numpy(),  # (174, )
                )
...
def train(
    ...
    for i, (media_type, (image, text, idx, roi_box_list, answer)) in enumerate(iterator):
        ...
        options_input = []
        for opt in options:
            options_input.append(tokenizer(opt).to(device))
        """
        ...
        # with torch.cuda.amp.autocast(enabled=config.use_half_precision, dtype=data_type):
            loss_dict = model(image, text_input, roi_box_list, answer, idx=idx)
            loss = loss_dict["loss_vtc"]
        optimizer.zero_grad()
...
def main(config):
    ...
    # 如果使用 bfloat16，则禁用 GradScaler
  
    if data_type == torch.bfloat16:
        logger.info("Detected bfloat16. Disabling GradScaler to avoid AMP issues...")
...
def eval_after_training(train_config):
    ...
    # train_config.pretrained_path = join(train_config.output_dir, "_ckpt.pth")
