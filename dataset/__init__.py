...
from dataset.ret_dataset import (ImgTxtRetTrainDataset,
                                 ...
                                 VidTxtImgRoiRetTrainDataset,
                                 VidImgRoiEvalDataset,   
                                 ssv1EvalDataset,
                                 )
# from dataset.ret_dataset import (ImgTxtRetTrainDataset,
#                                  VidTxtRetTrainDataset,
...
def get_dataset_cls(dataset_type, media_type, data_cfg):
    if dataset_type == "pt_train":
        ...
        elif media_type == "video_img":
            dataset_cls = VidTxtImgPtTrainDataset
        elif media_type == "ssv1_ori":
            dataset_cls = VidTxtImgPtTrainDataset
        ...
        if media_type == "image":
            ...
            dataset_cls = VidTxtImgRoiRetTrainDataset
        # elif media_type == 'audio':
        ...
        if media_type == "image":
            ...
            dataset_cls = VidImgRoiEvalDataset
        elif media_type == "ssv1_ori":
            dataset_cls = ssv1EvalDataset
        # elif media_type == "audio":
...
def create_dataset(dataset_type, config):
    ...
    if config.inputs.get('video_input', None) is not None and "imgs" not in config.train_corpus:
    ...
    elif "imgs" in config.train_corpus:
        video_reader_type = config.inputs.video_input.get("video_reader_type", "img")
        video_only_dataset_kwargs_train = dict(
            video_reader_type=video_reader_type,
            sample_type=config.inputs.video_input.sample_type,
            num_frames=config.inputs.video_input.num_frames,
    ...
    if dataset_type == "pt_train":
        ...
        if config.train_file.media_type == "video_img":
            dataset_kwargs = dict(
                ann_file=config.train_file,
                transform=train_transform)
            dataset_kwargs.update(video_only_dataset_kwargs_train)
        
        elif config.train_file.media_type == "ssv1_ori":
            dataset_kwargs = dict(
                ann_file=config.train_file,
                transform=train_transform)
            dataset_kwargs.update(video_only_dataset_kwargs_train)
        ...
        for name, data_cfg in config.test_file.items():
            ...
            if dataset_type == "qa_eval" or (dataset_type == "pt_eval" and "_qa_" in name):
                ...
                elif media_type == "video_img" or media_type == "ssv1_ori":
                    dataset_kwargs = dict(
                        ann_file=data_cfg,
                        transform=test_transform
                    )
        ...
        return ssv1EvalDataset(**dataset_kwargs)
