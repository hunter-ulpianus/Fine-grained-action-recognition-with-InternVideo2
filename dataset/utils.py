...
def load_anno(ann_file_list):
    ...
    for d in ann_file_list:        
        ...
        if os.path.splitext(fp)[-1] == ".json":
            cur_ann = json.load(open(fp, "r"))  # for val_part
        else:
            with open(fp, "r") as f:
                cur_ann = [json.loads(line) for line in f]
        ...
        for idx in iterator:
            if d.media_type == "image":
                ...
                key = "video"
            elif d.media_type in ["video_img", "ssv1_ori"]:
                key = "image"
            ...
            # for debug print(f"cur_ann{cur_ann[idx]}")
            if isinstance(cur_ann[idx][key], str):
                cur_ann[idx]["image"] = data_root_prefix + join(data_root, cur_ann[idx]["video"]) + ".webm"
            else:  # list
                cur_ann[idx]["box_root"] = os.path.join(d["box_root"], cur_ann[idx]["id"]) + ".json"
                cur_ann[idx]["answer"] = cur_ann[idx]["answer"]
                cur_ann[idx]["options"] = cur_ann[idx]["options"]
                cur_ann[idx]["data_root"] = os.path.join(data_root, cur_ann[idx]["id"])
        ann += cur_ann
    return ann
