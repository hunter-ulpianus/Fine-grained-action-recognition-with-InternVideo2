...
class ImgTxtRetTrainDataset(BaseDataset):
    ...
    def __init__(self, ann_file, transform):
        ...
        for ann in self.anno_list:
            ...
            key = ann["caption"] if self.has_multi_vision_gt else basename(ann["image"])
            if key not in self.match_ids:
                self.match_ids[key] = int(ann["answer"])
        self.match_ids = self.match_ids
    ...
    def __getitem__(self, index):
        try:
            ann = self.anno_list[index]
            image, index, roi_box_list = self.load_and_transform_media_data_with_box(index, ann["data_root"], ann["box_root"])
            caption = pre_text(ann["caption"])
            answer = int(ann["answer"])
            key = ann["caption"] if self.has_multi_vision_gt else basename(ann["image"])
            # options = [pre_text(e) for e in ann["options"]]
            # return image, caption, self.match_ids[key], roi_box_list, options
            return image, caption, self.match_ids[key], roi_box_list, answer
        except Exception as e:
            logger.error(e)
            print(e, flush=True)
            index = np.random.randint(0, len(self))
...
class VidTxtImgRoiRetTrainDataset(ImgTxtRetTrainDataset):
    media_type = "video_img"

    def __init__(
            self, ann_file, transform, num_frames=4,
            video_reader_type="img", sample_type="rand", num_tries=3):
        super(VidTxtImgRoiRetTrainDataset, self).__init__(ann_file, transform)
        self.num_frames = num_frames
        self.video_reader_type = video_reader_type
        self.video_reader = VIDEO_READER_FUNCS[video_reader_type]
        self.sample_type = sample_type
...
class ImgTxtRetEvalDataset(BaseDataset):
    ...
    def build_data_multi_img_gt(self):
        ...
        for txt_id, ann in enumerate(self.raw_anno_list):
            self.text.append(pre_text(ann["options"][int(ann["answer"])]))
            self.txt2img[txt_id] = []
            _images = ann["image"] \
                if isinstance(ann["image"], list) else [ann["image"], ]
...
class VidTxtRetMCNewEvalDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # image, index, roi_box_list = self.load_and_transform_media_data_with_box(index, ann["data_root"], ann["box_root"])
        image, index, roi_box_list = self.load_and_transform_media_data_with_box(index, ann["data_root"], ann["box_root"])
        option = [pre_text(e) for e in ann["options"]]  # len=174
        answer = int(ann["answer"])
        # answer = ann["template"].replace("[", "").replace("]", "")
        if isinstance(answer, list):
            answer = torch.Tensor(answer)
        return image, option, answer, ann, roi_box_list, answer

class ssv1EvalDataset(BaseDataset):
    media_type = "ssv1_ori"

    def __init__(self, ann_file, transform, num_frames=4,
                 video_reader_type="decord", sample_type="rand", num_tries=1):
        super(ssv1EvalDataset, self).__init__()
        self.anno_list = load_anno(ann_file)
        self.transform = transform
        # video args
        self.num_frames = num_frames
    ...
    def __getitem__(self, index):
        ann = self.anno_list[index]
        image, index, roi_box_list = self.load_and_transform_media_data_without_box(index, ann["data_root"])
        option = [pre_text(e) for e in ann["options"]]  # len=174
        answer = int(ann["answer"])
        # answer = ann["template"].replace("[", "").replace("]", "")
        if isinstance(answer, list):
...
class VidImgRoiEvalDataset(BaseDataset):
    """For ROI Align"""
    media_type = "video_img"

    def __init__(self, ann_file, transform, num_frames=4,
                 video_reader_type="img", sample_type="rand", num_tries=1):
        super(VidImgRoiEvalDataset, self).__init__()
        self.anno_list = load_anno(ann_file)
        self.transform = transform
        # video args
        self.num_frames = num_frames
    ...
    def __getitem__(self, index):
        ann = self.anno_list[index]
        image, index = self.load_and_transform_media_data(index, ann["data_root"])
        option = [pre_text(e) for e in ann["options"]]  # len=174
        answer = ann["answer"]
        # answer = ann["template"].replace("[", "").replace("]", "")
        if isinstance(answer, list):
