...
class ImgTxtPtTrainDataset(BaseDataset):
    ...
    def get_anno(self, index):
        ...
        anno["roi_box"] = os.path.join(self.box_root, anno["image"])
        return anno
    ...
    def __getitem__(self, index):
        try:
            ...
            elif ann.get("roi_box") is not None:
                image, index, roi_box_list = self.load_and_transform_media_data_video_frames_with_box(index, ann["image"], ann["roi_box"])
                return image, caption, index, roi_box_list
            else:
                image, index = self.load_and_transform_media_data(index, ann["image"])  #waiting to edit
...
class VidTxtImgPtTrainDataset(ImgTxtPtTrainDataset):
    media_type = "video"
    ...
    def get_anno(self, index):
        assert self.media_type == "video_img", self.media_type
        anno = {"caption": self.get_caption(index)}
        anno["image"] = self.data_root_prefix + os.path.join(self.data_root, self.anno[index]["id"])
        anno["roi_box"] = os.path.join(self.box_root, self.anno[index]["id"]) + ".json"
...
class VidTxtPtTrainDataset(ImgTxtPtTrainDataset):
    ...
    def get_anno(self, index):
        ...
        anno = {"caption": self.get_caption(index)}
        anno["video"] = self.data_root_prefix + os.path.join(self.data_root, self.anno[index]["video"]) + ".webm"
        if self.read_clip_from_video:
            anno["video_start_frame"] = self.anno[index]["video_start_frame"]
            anno["video_end_frame"] = self.anno[index]["video_end_frame"]
        if self.use_prompt:
